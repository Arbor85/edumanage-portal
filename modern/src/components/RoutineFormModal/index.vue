...
import type {
  ...
  SupersetGroup, SupersetColor, DropConfig, PlanWorkoutInput,
} from '../../types'
import { useRoutineStore } from '../../stores/routineStore'
import { useToast } from '../../composables/useToast'
import FullSizeDialog from '../FullSizeDialog/index.vue'
import ExercisePickerDialog from '../ExercisePickerDialog/index.vue'
...
const isEmbedded = computed(() => props.embeddedWorkout !== undefined)

usePageTitle(
  () => isEmbedded.value ? 'Edit Workout' : (props.routine ? 'Edit Routine' : 'New Routine'),
  () => props.open,
)
...
watch(() => props.open, (val) => {
  if (val) {
    if (isEmbedded.value && props.embeddedWorkout) {
      const exercises = (props.embeddedWorkout.excercises ?? []).map(ex => ({
        ...ex,
        sets: (ex.sets ?? []).map(s => ({ ...s })),
      }))
      const groupIds = [...new Set(
        exercises.map(e => e.supersetGroupId).filter((id): id is string => Boolean(id))
      )]
      const supersetGroups: SupersetGroup[] = groupIds.map((id, i) => ({
        id,
        name: null,
        color: COLOR_ORDER[i % COLOR_ORDER.length],
      }))
      form.value = {
        name: props.embeddedWorkout.name,
        note: props.embeddedWorkout.note,
        excercises: exercises,
        supersetGroups,
      }
      nameIsAuto.value = false
    } else if (props.routine) {
      form.value = {
...
async function save() {
  if (isEmbedded.value && props.embeddedWorkout !== undefined) {
    emit('save', {
      ...props.embeddedWorkout,
  ...
  <FullSizeDialog :open="open">
    <template #header>
      <input
        ...
        @input="onNameInput"
        type="text"
        :placeholder="isEmbedded ? 'Workout name…' : 'Routine name…'"
        autofocus
    ...
    <template #footer>
      <div class="flex items-center gap-2">
        <BaseButton v-if="!isEmbedded && routine" variant="danger" @click="confirmDelete = true">Delete</BaseButton>
        <BaseButton v-if="!isEmbedded && !routine" variant="ghost" @click="isDefaultWorkoutPickerOpen = true">Use default workout</BaseButton>
        <div class="flex-1" />
        <BaseButton variant="ghost" @click="requestClose">Cancel</BaseButton>
        <BaseButton variant="primary" :loading="saving" @click="save">
          {{ isEmbedded ? 'Done' : (routine ? 'Save' : 'Create') }}
        </BaseButton>
      </div>
    </template>
  </FullSizeDialog>
