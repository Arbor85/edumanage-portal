...
watch(() => props.open, (val) => {
  ...
  sets.value = props.initialSets?.length
    ? props.initialSets.map(s => ({ reps: s.reps, weight: s.weight }))
    : [{ reps: null, weight: null }]
})

function addSet() {
  const last = sets.value[sets.value.length - 1]
  sets.value.push({ reps: last?.reps ?? null, weight: last?.weight ?? null })
}
...
function confirm() {
  emit('confirm', sets.value.map(s => ({ reps: s.reps ?? null, weight: s.weight ?? null })))
  emit('close')
}
</script>
...
<template>
  <BaseModal :open="open" :title="exerciseName" size="sm" @close="emit('close')">
    <div class="flex flex-col gap-4">
      <div class="grid gap-2 text-xs font-medium text-text-secondary px-1"
        :class="isBodyweight ? 'grid-cols-[2rem_1fr_2rem]' : 'grid-cols-[2rem_1fr_1fr_2rem]'"
      >
        <span class="text-center">#</span>
        <span>Reps</span>
        <span v-if="!isBodyweight">Weight (kg)</span>
        <span v-else />
        <span />
      </div>
      ...
      <button
        type="button"
        class="flex items-center gap-1.5 text-sm text-primary hover:underline self-start"
        @click="addSet"
      >
