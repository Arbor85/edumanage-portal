...
const selected = computed(() =>
  routineStore.routines.find((r) => r.id === props.modelValue) ?? null
)
      ...
      <ul class="max-h-48 overflow-y-auto custom-scrollbar py-1">
        <li v-for="routine in filtered" :key="routine.id ?? ''">
          <button
            ...
            :class="modelValue === routine.id ? 'bg-primary/10' : ''"
            @click="pick(routine.id)"
          >
            <span class="flex-1 text-left truncate">{{ routine.name }}</span>
            <span class="text-xs text-text-secondary">{{ routine.excercises?.length ?? 0 }} exercises</span>
          </button>
        </li>
        <li v-if="!filtered.length" class="px-3 py-4 text-sm text-center text-text-secondary">No routines found</li>
      </ul>
    </div>
