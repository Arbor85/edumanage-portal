...
import { ref } from 'vue'
import { ChevronDown, ChevronUp, Plus, Trash2, Pencil, Building2 } from 'lucide-vue-next'
import BaseButton from '../../../../components/BaseButton.vue'
import BaseInput from '../../../../components/BaseInput.vue'
import BaseBadge from '../../../../components/BaseBadge.vue'
import type { BuildingOut, BuildingAvailabilityOut, BuildingAvailabilityCreate } from '../../../../types'

const props = defineProps<{
  building: BuildingOut
  availabilities: BuildingAvailabilityOut[]
}>()

const emit = defineEmits<{
  edit: [building: BuildingOut]
  delete: [id: string]
  addAvailability: [payload: BuildingAvailabilityCreate]
...
const form = ref<BuildingAvailabilityCreate>({
  ...
  <div class="bg-surface dark:bg-surface-card rounded-2xl border border-gray-100 dark:border-white/5 overflow-hidden shadow-sm">
    ...
    >
      ...
      <div class="flex-1 min-w-0">
        <p class="text-sm font-semibold text-text-primary dark:text-white">{{ building.name }}</p>
        <p class="text-xs text-text-secondary mt-0.5 truncate">{{ building.address }}</p>
      </div>
      <div class="flex items-center gap-2 flex-shrink-0">
        <BaseBadge :label="`${building.capacity} seats`" />
        <BaseButton size="sm" variant="ghost" @click.stop="emit('edit', building)">
          <Pencil class="w-3.5 h-3.5" />
        </BaseButton>
        <BaseButton size="sm" variant="ghost" @click.stop="emit('delete', building.id)">
