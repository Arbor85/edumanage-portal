...
const REGION_ALIASES: Record<MuscleRegion, string[]> = {
  ...
  tibialis: ['tibialis', 'tibialis anterior', 'calves'],
  calves: ['calves'],
  upperBack: ['upper back', 'upper-back'],
  midBack: ['middle back', 'mid back', 'midBack', 'mid-back'],
  lowerBack: ['lower back', 'lowerBack', 'lower-back'],
  lats: ['lats', 'latissimus', 'upper back', 'middle back', 'mid back'],
  glutes: ['glutes', 'glute', 'gluteus'],
  hamstrings: ['hamstrings', 'hams'],
  traps: ['traps', 'trapezius'],
  rearDelts: ['rear delts', 'rear deltoids', 'shoulders', 'delts', 'deltoids', 'rearDelts', 'rear-delts'],
};

const normalize = (value: string) => value.trim().toLowerCase();

const pickRegions = (muscles: string[] | undefined) => {
  const list = (muscles ?? []).map(normalize);
  const active = new Set<MuscleRegion>();
  (Object.keys(REGION_ALIASES) as MuscleRegion[]).forEach((region) => {
    const aliases = REGION_ALIASES[region];
    if (aliases.some((alias) => list.includes(alias))) {
      active.add(region);
    }
  });
  return active;
...
const HumanMuscleMap: React.FC<HumanMuscleMapProps> = ({ primaryMuscles, secondaryMuscles }) => {
  const primaryRegions = useMemo(() => pickRegions(primaryMuscles), [primaryMuscles]);
  const secondaryRegions = useMemo(() => pickRegions(secondaryMuscles), [secondaryMuscles]);
  const svgFrontContainerId = `muscles-svg-front-${Math.random().toString(36).slice(2)}`;
  const svgBackContainerId = `muscles-svg-back-${Math.random().toString(36).slice(2)}`;

  useEffect(() => {
    const applyStyles = (containerId: string) => {
      const container = document.getElementById(containerId);
      if (!container) return;

      const muscleIds = Object.keys(REGION_ALIASES) as MuscleRegion[];
      
      muscleIds.forEach((regionId) => {
        const elements = container.querySelectorAll(`#${regionId}`);
        elements.forEach((el) => {
          ...
          if (primaryRegions.has(regionId)) {
            fillColor = 'rgba(59, 130, 246, 0.7)'; // blue
            strokeColor = 'rgb(59, 130, 246)';
          } else if (secondaryRegions.has(regionId)) {
            fillColor = 'rgba(251, 146, 60, 0.6)'; // amber
            strokeColor = 'rgb(245, 158, 11)';
          } else {
            fillColor = 'rgba(203, 213, 225, 0.6)'; // slate
    ...
    applyStyles(svgFrontContainerId);
    applyStyles(svgBackContainerId);
  }, [primaryRegions, secondaryRegions, svgFrontContainerId, svgBackContainerId]);

  return (
    <div className="rounded-2xl border border-slate-200 dark:border-border-dark bg-slate-50 dark:bg-slate-900/40 p-4">
      <div className="flex justify-center gap-4 max-w-[600px] mx-auto">
        <div className="flex-1 max-w-[300px]">
          <h3 className="text-center text-sm font-semibold mb-2 text-slate-700 dark:text-slate-300">Front</h3>
          <div 
            id={svgFrontContainerId}
            className="flex justify-center"
            dangerouslySetInnerHTML={{ __html: MUSCLES_SVG }}
          />
        </div>
        <div className="flex-1 max-w-[300px]">
          <h3 className="text-center text-sm font-semibold mb-2 text-slate-700 dark:text-slate-300">Back</h3>
          <div 
            id={svgBackContainerId}
            className="flex justify-center"
            dangerouslySetInnerHTML={{ __html: MUSCLES_BACK_SVG }}
          />
        </div>
      </div>

      <div className="mt-4 flex flex-wrap items-center justify-center gap-3 text-[10px] font-bold uppercase tracking-[0.2em]">
