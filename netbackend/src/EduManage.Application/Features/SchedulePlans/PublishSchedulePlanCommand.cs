...
public sealed record PublishSchedulePlanCommand(string PlanId) : IRequest<SchedulePlanOut>
{
    internal sealed class Handler(ISchedulePlanRepository repo) : IRequestHandler<PublishSchedulePlanCommand, SchedulePlanOut>
    {
        public async Task<SchedulePlanOut> Handle(PublishSchedulePlanCommand request, CancellationToken cancellationToken)
        {
