<template>
  <div v-if="isAuthenticated" ref="containerRef" class="relative">
    <div
      ...
      class="h-10 w-10 rounded-full overflow-hidden border border-gray-300 bg-white flex items-center justify-center"
    ...
    <div v-if="isOpen" class="absolute right-0 top-full pt-2 z-50">
      <div class="w-72 rounded-lg border border-gray-200 bg-white shadow p-4">
        <div class="flex items-center gap-3 mb-3">
          <img
            :src="displayImage"
            :alt="user?.name || 'Profile picture'"
            class="block h-10 w-10 rounded-full object-cover border border-gray-200"
            referrerpolicy="no-referrer"
            @error="onImageError"
          />
          <div>
            <p class="font-semibold text-gray-900">{{ user?.name || user?.nickname || 'User' }}</p>
        ...
        <dl class="text-sm space-y-2">
          <div class="flex justify-between gap-3">
            ...
            <dd class="text-gray-900 text-right">{{ user?.nickname || '-' }}</dd>
          </div>
          <div class="flex justify-between gap-3">
