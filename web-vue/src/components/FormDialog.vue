<template>
  <div v-if="open" class="fixed inset-0 z-[70] flex items-center justify-center bg-slate-900/50 p-4" @click.self="$emit('cancel')">
    <div :class="maxWidthClass" class="w-full rounded-lg border border-slate-200 bg-white/40 shadow-lg hover:shadow-2xl focus-within:shadow-2xl transition-shadow dark:border-slate-700 dark:bg-slate-800/40 flex flex-col">
      <div class="px-5 py-4 flex items-center justify-between border-b border-slate-300 backdrop-blur-sm dark:border-slate-600">
      ...
      <form class="p-5 space-y-4 backdrop-blur-lg" @submit.prevent="$emit('submit')">
        <slot />
